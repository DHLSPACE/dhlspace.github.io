...
def published_date(node):
    ...
    for _ in range(4):
        ...
        # Some sites display day above year.month, including inside a linked date block.
        for part in parent.walk():
            if part.tag in {'div','span','time'} and re.search('date|time',part.attrs.get('class',''),re.I):
                text = clean(part.text())
                reverse = re.fullmatch(r'(\d{1,2})\s+(20\d{2})[./-](\d{1,2})',text)
                if reverse:
                    d,y,m=map(int,reverse.groups())
                    try:
