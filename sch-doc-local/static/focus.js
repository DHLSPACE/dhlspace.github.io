...
$('nav [data-tab="settings"]').innerHTML='<span aria-hidden="true">⚙</span>';
$('nav [data-tab="settings"]').classList.add('settings-gear');
$('aside').append($('nav [data-tab="settings"]'));
$('aside').setAttribute('aria-label','主导航');
$('.brand').innerHTML='<span class="logo">研</span><div>化工研途<small>GRADUATE ATLAS</small></div>';
...
renderDirectory=()=>{
  ...
  $('#directory-list').innerHTML=rows.slice((directoryPage-1)*pageSize,directoryPage*pageSize).map(u=>{
    ...
    return `<article class="institution-card"><div class="institution-top"><span class="institution-emblem">${u.tier==='研究所'?'CAS':u.tier==='985'?'985':'UNI'}</span><span class="meta">${esc(u.province)} · ${esc(u.fit)}</span><button class="pin ${favorites.has(u.id)?'selected':''}" data-favorite="${u.id}" aria-label="${favorites.has(u.id)?'取消关注':'关注'} ${esc(u.school)}" aria-pressed="${favorites.has(u.id)}">${favorites.has(u.id)?'★':'☆'}</button></div><h3>${esc(u.school)}</h3><div class="direction-tags">${u.directions.map(d=>`<span>${esc(d)}</span>`).join('')}</div><p class="entry-status">${esc(u.entry_status)}</p><div class="institution-meta"><span>${successful}/${sources.length} 来源最近检查成功</span><span>${saved} 个相关版本</span></div><details class="institution-entries"><summary>校院入口 <span>${u.entries.length}</span></summary>${u.entries.map(entryLink).join('')}<p class="hint">常设入口仅作导航，不自动保存综合新闻；公告是否长期保留尚未确认。</p></details><div class="institution-actions"><button data-connect="${u.id}">补充来源</button><button data-school-files="${u.id}" ${saved?'':'disabled'}>资料 ${saved}</button>${link(u.url,'官网')}</div></article>`;
...
renderLibrary=()=>{
  ...
  const q=$('#archive-search').value.trim().toLowerCase(),seen=new Set(),all=data.snapshots.filter(x=>{const key=x.source_id+'|'+x.url;if($('#archive-latest').checked&&seen.has(key))return false;seen.add(key);return true});
  const rows=all.filter(x=>($('#archive-unrelated').checked||x.filing.relevant)&&(!q||(archiveTitle(x)+x.url+x.filing.school+x.filing.department).toLowerCase().includes(q))&&['scope','year','school','level'].every(k=>!$('#archive-'+k).value||x.filing[k]===$('#archive-'+k).value)&&(!$('#archive-format').value||fileFormat(x)===$('#archive-format').value));
